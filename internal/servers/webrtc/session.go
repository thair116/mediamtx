...
func (s *session) runPublish(req *initialRequestReq) (int, error) {
	...
	pc := &webrtc.PeerConnection{
		...
		ICEDisconnectedTimeout: time.Duration(s.iceDisconnectedTimeout),
		ICEFailedTimeout:       time.Duration(s.iceFailedTimeout),
		ICEKeepaliveInterval:   time.Duration(s.iceKeepaliveInterval),
		Publish:                false,
		Log:                    s,
	}
	err = pc.Start()
